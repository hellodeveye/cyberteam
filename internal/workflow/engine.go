...
// 类型别名 - 从 protocol 包导出以保持向后兼容
type Stage = protocol.Stage
type Status = protocol.Status
type Task = protocol.WorkflowTask
type Project = protocol.Project

const (
	StageRequirement = protocol.StageRequirement
	StageDesign      = protocol.StageDesign
	StageReview      = protocol.StageReview
	StageDevelop     = protocol.StageDevelop
	StageTest        = protocol.StageTest
	StageDeploy      = protocol.StageDeploy
	StageDone        = protocol.StageDone
)

const (
	StatusPending    = protocol.StatusPending
	StatusAssigned   = protocol.StatusAssigned
	StatusInProgress = protocol.StatusInProgress
	StatusCompleted  = protocol.StatusCompleted
	StatusRejected   = protocol.StatusRejected
